...
const generateUniqueSlug = async (title) => {
  ...
  while (!isUnique) {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      throw new Error('The title is not unique add or remove some characters: ' + error.message);
    }
...
export default function UploadPage() {
  ...
  // Function to compress image using the API route
  const compressImage = async (imageFile) => {
    ...
    return new File([compressedImageBlob], imageFile.name.replace(/\.\w+$/, '.webp'), {
      type: 'image/webp',
    });
  };
  ...
  const handleDocumentUpload = async (e) => {
    ...
    try {
      ...
      if (gettingUserError) {
        console.error('Error while fetching user_id:', gettingUserError);
      ...
      // Compress the cover image using the API route
      const compressedCoverImage = await compressImage(coverImage);

      // Upload compressed cover image to Supabase Storage
      const coverImageExt = compressedCoverImage.name.split('.').pop(); // Get image extension
      ...
      const { data: coverImageData, error: coverImageError } = await supabase.storage
        .from('cover-images') // bucket name
        .upload(coverImageName, compressedCoverImage);
        ...
        .insert([
          {
            ...
            file_url: file_url,
            tags: tags.split(',').map((tag) => tag.trim().toLowerCase()), // Convert tags to an array
          },
        ])
        .single();
